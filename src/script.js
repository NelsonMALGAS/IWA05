...
if (custlocation == 'RSA') {
     shipping = '400'; 
     currency == 'R'; // Updae currency for RSA custlocation
...
if (shoes + batteries + pens + shirts + toys > 1000 && currency==='$' ) {
	if (locationA ==' NAM' && customers < 2) {
        shipping='800'; //update shipping for NAM location and less than 2 customres
			if (custlocation == 'RSA'){
		    shipping = 0 || calcShipping();//use correct function name
		}
	}
}
